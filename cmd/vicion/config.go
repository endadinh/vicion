...
type vicionConfig struct {
	Eth         eth.Config
	Shh         whisper.Config
	Node        node.Config
	Ethstats    ethstatsConfig
	VicionX     vicionx.Config
	Account     account
	StakeEnable bool
	Bootnodes   Bootnodes
	Verbosity   int
...
func loadConfig(file string, cfg *vicionConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "vicion.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, vicionConfig) {
	// Load defaults.
	cfg := vicionConfig{
		Eth:         eth.DefaultConfig,
		Shh:         whisper.DefaultConfig,
		VicionX:     vicionx.DefaultConfig,
	...
	if ctx.GlobalBool(utils.TomoTestnetFlag.Name) {
		...
		common.TIPTRC21Fee = common.TIPVicionXTestnet
	...
	utils.SetVicionXConfig(ctx, &cfg.VicionX, cfg.Node.DataDir)
	return stack, cfg
}
...
func makeFullNode(ctx *cli.Context) (*node.Node, vicionConfig) {
	stack, cfg := makeConfigNode(ctx)

	// Register VicionX's OrderBook service if requested.
	// enable in default
	utils.RegisterVicionXService(stack, &cfg.VicionX)
