...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a vicion instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		vicion := runTomo(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		vicion.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "vicion", "chaindata")
