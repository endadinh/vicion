...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The vicion monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	if client, err = dialRPC(endpoint); err != nil {
		utils.Fatalf("Unable to attach to vicion node: %v", err)
	}
	defer client.Close()
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by vicion (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached vicion node and retrieves the entire set
