...
contract AbstractVICIONXListing {
...
contract RelayerRegistration {
    ...
    address constant private vicionNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractVICIONXListing private VicionXListing;
    ...
    constructor (address vicionxListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        VicionXListing = AbstractVICIONXListing(vicionxListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with vicion(x/VICION)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 VICION is required for a deposit request");
    ...
    function indexOf(address[] memory vicionPair, address target) internal pure returns (bool){
        for (uint i = 0; i < vicionPair.length; i ++) {
            if (vicionPair[i] == target) {
                return true;
            }
        }
        return false;
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory vicionPairs = new address[](fromTokens.length);
        address[] memory nonTomoPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = VicionXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == vicionNative);
            b = b && (VicionXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == vicionNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == vicionNative) {
                vicionPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == vicionNative) {
                    vicionPairs[countPair] = toTokens[i];
                    countPair++;
                }
        ...
        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(vicionPairs, nonTomoPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory vicionPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = VicionXListing.getTokenStatus(toToken) || (toToken == vicionNative);
        b = b && (VicionXListing.getTokenStatus(fromToken) || fromToken == vicionNative);
        if (!b) {
            return false;
        }

        if (fromToken == vicionNative || toToken == vicionNative) {
            return true;
        }

        // get tokens that paired with vicion
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == vicionNative) {
                vicionPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == vicionNative) {
                    vicionPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(vicionPairs, toToken)) {
            return false;
        }
        return true;
    }
