...
import (
	...
	"github.com/vicion/vicion/contracts/vicionx"
	simulation "github.com/vicion/vicion/contracts/vicionx/testnet"
...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := vicionx.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	//init VICIONX Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	vicionxListtingAddr, vicionxListing, err := vicionx.DeployVICIONXListing(auth, client)
	if err != nil {
		log.Fatal("DeployVICIONXListing", err)
	}
	vicionxListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> vicionx listing address", vicionxListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : vicionx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := vicionx.DeployRelayerRegistration(auth, client, vicionxListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := vicionx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, vicionxListtingAddr)
	if err != nil {
	...
	applyVicionXListing(vicionxListing, tokenList, currentNonce)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddCollateral(simulation.VICIONNative, simulation.CollateralDepositRate, simulation.CollateralLiquidationRate, simulation.CollateralRecallRate)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddBaseToken(simulation.VICIONNative)
	if err != nil {
		log.Fatal("Lending add base token VICION", err)
	}
	...
	relayerRegistration, err = vicionx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	/*
		for _, token := range tokenList {
			...
			toTokens = append(toTokens, simulation.VICIONNative)
		}
	*/

	// VICION/BTC
	fromTokens = append(fromTokens, simulation.VICIONNative)
	...
	fmt.Println("wait 2s to apply token to list vicionx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = vicionx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	...
	// VICION 1 min
	baseTokens = append(baseTokens, simulation.VICIONNative)
	terms = append(terms, big.NewInt(60))
	collaterals = append(collaterals, common.HexToAddress("0x0"))
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		if tokenName == "ADA" {
			d = 0
			tokenCap = new(big.Int).Div(simulation.TRC21TokenCap, simulation.BaseVICION)
		}
		if tokenName == "USDT" {
			d = 6
		...
		tokenAddr, _, err := vicionx.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *vicionx.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyVicionXListing(vicionxListing *vicionx.VICIONXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		vicionxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		vicionxListing.TransactOpts.Value = simulation.VicionXListingFee
		_, err := vicionxListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("vicionxListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyVicionXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := vicionx.NewTRC21(auth, token["address"].(common.Address), client)
