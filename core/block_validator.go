...
func (v *BlockValidator) ValidateTradingOrder(statedb *state.StateDB, vicionxStatedb *tradingstate.TradingStateDB, txMatchBatch tradingstate.TxMatchBatch, coinbase common.Address, header *types.Header) error {
	posvEngine, ok := v.bc.Engine().(*posv.Posv)
	if posvEngine == nil || !ok {
		return ErrNotPoSV
	}
	vicionXService := posvEngine.GetVicionXService()
	if vicionXService == nil {
		return fmt.Errorf("vicionx not found")
	}
	...
	for _, txMatch := range txMatchBatch.Data {
		...
		newTrades, newRejectedOrders, err := vicionXService.ApplyOrder(header, coinbase, v.bc, statedb, vicionxStatedb, tradingstate.GetTradingOrderBookHash(order.BaseToken, order.QuoteToken), order)
		if err != nil {
			return err
		}
	...
	if vicionXService.IsSDKNode() {
		v.bc.AddMatchingResult(txMatchBatch.TxHash, tradingResult)
	}
	return nil
}

func (v *BlockValidator) ValidateLendingOrder(statedb *state.StateDB, lendingStateDb *lendingstate.LendingStateDB, vicionxStatedb *tradingstate.TradingStateDB, batch lendingstate.TxLendingBatch, coinbase common.Address, header *types.Header) error {
	posvEngine, ok := v.bc.Engine().(*posv.Posv)
	if posvEngine == nil || !ok {
		return ErrNotPoSV
	}
	...
	for _, l := range batch.Data {
		...
		// process Matching Engine
		newTrades, newRejectedOrders, err := lendingService.ApplyOrder(header, coinbase, v.bc, statedb, lendingStateDb, vicionxStatedb, lendingstate.GetLendingOrderBookHash(l.LendingToken, l.Term), l)
		if err != nil {
			return err
		}
