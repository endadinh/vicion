...
import (
	...
	"github.com/vicion/vicion/common/mclock"
	...
	"github.com/vicion/vicion/ethclient"
	"github.com/vicion/vicion/ethdb"
	"github.com/vicion/vicion/event"
	"github.com/vicion/vicion/log"
	"github.com/vicion/vicion/metrics"
	"github.com/vicion/vicion/params"
	"github.com/vicion/vicion/rlp"
	"github.com/vicion/vicion/trie"
...
func NewBlockChainEx(db ethdb.Database, vicionxDb ethdb.TomoxDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addTomoxDb(vicionxDb)
	}
...
func (bc *BlockChain) addTomoxDb(vicionxDb ethdb.TomoxDatabase) {
	bc.vicionxDb = vicionxDb
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		if ok {
			tradingService := engine.GetVicionXService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPVicionX(currentBlock.Number()) && bc.chainConfig.Posv != nil && currentBlock.NumberU64() > bc.chainConfig.Posv.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*posv.Posv)
	if ok {
		vicionXService := engine.GetVicionXService()
		if bc.Config().IsTIPVicionX(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && vicionXService != nil {
			...
			if err == nil {
				return vicionxState, nil
			} else {
				return nil, err
			}
		}
	}
	return nil, errors.New("Get vicionx state fail")
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPVicionX(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetVicionXService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPVicionX((*head).Number()) && bc.chainConfig.Posv != nil && (*head).NumberU64() > bc.chainConfig.Posv.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	if bc.Config().IsTIPVicionX(block.Number()) && bc.chainConfig.Posv != nil && engine != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch {
		tradingService := engine.GetVicionXService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) SaveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPVicionX(bc.CurrentBlock().Number()) && bc.chainConfig.Posv != nil && bc.CurrentBlock().NumberU64() > bc.chainConfig.Posv.Epoch && engine != nil {
			tradingService = engine.GetVicionXService()
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPVicionX(recent.Number()) && bc.chainConfig.Posv != nil && recent.NumberU64() > bc.chainConfig.Posv.Epoch && engine != nil {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	var lendingTrieDb *trie.Database
	var lendingService posv.LendingService
	if bc.Config().IsTIPVicionX(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && engine != nil {
		tradingService = engine.GetVicionXService()
		if tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
			...
			if tradingService != nil && lendingService != nil {
				...
				if tradingState != nil && tradingService != nil {
					...
					if gotRoot != expectRoot {
						err = fmt.Errorf("invalid vicionx trading state merke trie got : %s , expect : %s ,parent : %s", gotRoot.Hex(), expectRoot.Hex(), parentRoot.Hex())
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
					log.Debug("VicionX Trading State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
				if lendingState != nil && tradingState != nil {
					...
					log.Debug("VicionX Lending State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
			}
		}
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPVicionX(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if bc.chainConfig.IsTIPVicionX(commonBlock.Number()) && bc.chainConfig.Posv != nil && commonBlock.NumberU64() > bc.chainConfig.Posv.Epoch {
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	vicionXService := engine.GetVicionXService()
	if vicionXService == nil || !vicionXService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractTradingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			...
			if err := vicionXService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
...
func (bc *BlockChain) reorgTxMatches(deletedTxs types.Transactions, newChain types.Blocks) {
	...
	for _, deletedTx := range deletedTxs {
		if deletedTx.IsTradingTransaction() {
			...
			if err := vicionXService.RollbackReorgTxMatch(deletedTx.Hash()); err != nil {
