...
// blockChain_vicionx add order state
...
func (pool *LendingPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPVicionX(pool.chain.CurrentBlock().Number()) || pool.chain.Config().Posv == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().Posv.Epoch {
		return
	}
...
func (pool *LendingPool) validateBalance(cloneStateDb *state.StateDB, cloneLendingStateDb *lendingstate.LendingStateDB, tx *types.LendingTransaction, collateralToken common.Address) error {
	...
	vicionXServ := posvEngine.GetVicionXService()
	lendingServ := posvEngine.GetLendingService()
	if vicionXServ == nil {
		return fmt.Errorf("vicionx not found in order validation")
	}
	lendingTokenDecimal, err := vicionXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.LendingToken())
	if err != nil {
	...
	tradingStateDb, err := vicionXServ.GetTradingState(pool.chain.CurrentBlock(), author)
	if err != nil {
	...
	// lendTokenVICIONPrice: price of lendingToken in VICION quote
	var lendTokenVICIONPrice, collateralPrice, collateralTokenDecimal *big.Int
	if collateralToken.String() != lendingstate.EmptyAddress {
		collateralTokenDecimal, err = vicionXServ.GetTokenDecimal(pool.chain, cloneStateDb, collateralToken)
		if err != nil {
			return fmt.Errorf("validateOrder: failed to get collateralTokenDecimal. err: %v", err)
		}
		lendTokenVICIONPrice, collateralPrice, err = lendingServ.GetCollateralPrices(pool.chain.CurrentHeader(), pool.chain, cloneStateDb, cloneTradingStateDb, collateralToken, tx.LendingToken())
		if err != nil {
			return err
		}
		if lendTokenVICIONPrice == nil || lendTokenVICIONPrice.Sign() <= 0 || collateralPrice == nil || collateralPrice.Sign() <= 0 {
			log.Debug("ValidateLending: ErrInvalidCollateralPrice", "lendTokenVICIONPrice", lendTokenVICIONPrice, "collateralPrice", collateralPrice)
	...
	if lendTokenVICIONPrice == nil || lendTokenVICIONPrice.Sign() == 0 {
		if tx.LendingToken().String() == common.TomoNativeAddress {
			lendTokenVICIONPrice = common.BasePrice
		} else {
			lendTokenVICIONPrice, err = lendingServ.GetMediumTradePriceBeforeEpoch(pool.chain, cloneStateDb, cloneTradingStateDb, tx.LendingToken(), common.HexToAddress(common.TomoNativeAddress))
			if err != nil {
				return err
			}
		}
	}
	isVicionXLendingFork := pool.chain.Config().IsTIPVicionXLending(pool.chain.CurrentHeader().Number)
	if err := lendingstate.VerifyBalance(isVicionXLendingFork,
		cloneStateDb,
		cloneLendingStateDb,
		tx.Type(),
		tx.Side(),
...
func (pool *LendingPool) addTx(tx *types.LendingTransaction, local bool) error {
	if !pool.chainconfig.IsTIPVicionX(pool.chain.CurrentBlock().Number()) {
		return nil
	}
	tx.CacheHash()
