...
type OrderPool struct {
	...
	pendingState      *tradingstate.VicionXManagedState // Pending state tracking virtual nonces
...
func (pool *OrderPool) State() *tradingstate.VicionXManagedState {
	pool.mu.RLock()
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	cloneVicionXStateDb := pool.currentOrderState.Copy()
	...
	if !tx.IsCancelledOrder() {
		...
		if orderType == OrderTypeLimit {
			...
			baseDecimal, err := vicionXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.BaseToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get baseDecimal. err: %v", err)
			}
			quoteDecimal, err := vicionXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.QuoteToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get quoteDecimal. err: %v", err)
			}
			if err := tradingstate.VerifyBalance(cloneStateDb, cloneVicionXStateDb, tx, baseDecimal, quoteDecimal); err != nil {
				return err
			}
		}
	...
	if !tx.IsCancelledOrder() {
		...
		originOrder := cloneVicionXStateDb.GetOrder(tradingstate.GetTradingOrderBookHash(tx.BaseToken(), tx.QuoteToken()), common.BigToHash(new(big.Int).SetUint64(tx.OrderID())))
