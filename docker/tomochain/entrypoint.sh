...
params=""
accountsCount=$(
  vicion account list --datadir $DATA_DIR  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    ...
    89 )
      ...
      params="$params --vicion-testnet --gcmode archive --rpcapi db,eth,net,web3,debug,posv"
      ;;
    90 )
...
# data dir
if [[ ! -d $DATA_DIR/vicion ]]; then
  echo "No blockchain data, creating genesis block."
  vicion init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    vicion  account import ./private_key \
      --datadir $DATA_DIR \
