...
import (
	...
	stateDatabase "github.com/vicion/vicion/core/state"
...
func (b *EthApiBackend) GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, vicionxState *tradingstate.TradingStateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error) {
	state.SetBalance(msg.From(), math.MaxBig256)
	vmError := func() error { return nil }

	context := core.NewEVMContext(msg, header, b.eth.BlockChain(), nil)
	return vm.NewEVM(context, state, vicionxState, b.eth.chainConfig, vmCfg), vmError, nil
}
...
func (b *EthApiBackend) GetOrderNonce(address common.Hash) (uint64, error) {
	vicionxService := b.eth.GetVicionX()
	if vicionxService != nil {
		author, err := b.GetEngine().Author(b.CurrentBlock().Header())
		if err != nil {
			return 0, err
		}
		vicionxState, err := vicionxService.GetTradingState(b.CurrentBlock(), author)
		if err != nil {
			return 0, err
		}
		return vicionxState.GetNonce(address), nil
	}
	return 0, errors.New("cannot find vicionx service")
}

func (b *EthApiBackend) TomoxService() *vicionx.VicionX {
	return b.eth.VicionX
}

func (b *EthApiBackend) LendingService() *vicionxlending.Lending {
