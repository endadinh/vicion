...
type Backend interface {
	...
	TomoxService() *vicionx.VicionX
	LendingService() *vicionxlending.Lending
	...
	GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, vicionxState *tradingstate.TradingStateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error)
	...
	return []rpc.API{
		{
			...
			Namespace: "vicionx",
			Version:   "1.0",
			Service:   NewPublicVicionXTransactionPoolAPI(apiBackend, nonceLock),
			Public:    true,
		}, {
			Namespace: "txpool",
			Version:   "1.0",
