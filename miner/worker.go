...
import (
	...
	"github.com/vicion/vicion/consensus/misc"
	"github.com/vicion/vicion/consensus/posv"
	"github.com/vicion/vicion/contracts"
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	var vicionxState *tradingstate.TradingStateDB
	var lendingState *lendingstate.LendingStateDB
	if self.config.Posv != nil {
		vicionX := self.eth.GetVicionX()
		vicionxState, err = vicionX.GetTradingState(parent, author)
		if err != nil {
			log.Error("Failed to get vicionx state ", "number", parent.Number(), "err", err)
			return err
		}
		lending := self.eth.GetVicionXLending()
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.Posv != nil && self.chain.Config().IsTIPVicionX(header.Number) {
			vicionX := self.eth.GetVicionX()
			vicionXLending := self.eth.GetVicionXLending()
			if vicionX != nil && header.Number.Uint64() > self.config.Posv.Epoch {
				if header.Number.Uint64()%self.config.Posv.Epoch == 0 {
					err := vicionX.UpdateMediumPriceBeforeEpoch(header.Number.Uint64()/self.config.Posv.Epoch, work.tradingState, work.state)
					if err != nil {
				...
				//https://github.com/vicion/vicion-v1/pull/416
				if header.Number.Uint64()%self.config.Posv.Epoch != 0 {
					...
					tradingTxMatches, tradingMatchingResults = vicionX.ProcessOrderPending(header, self.coinbase, self.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = vicionXLending.ProcessOrderPending(header, self.coinbase, self.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%self.config.Posv.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = vicionXLending.ProcessLiquidationData(header, self.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(tradingTxMatches) > 0 {
					...
					tx := types.NewTransaction(nonce, common.HexToAddress(common.VicionXAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					lendingTx := types.NewTransaction(nonce, common.HexToAddress(common.VicionXLendingAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.HexToAddress(common.VicionXLendingFinalizedTradeAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		// validate balance slot, token decimal for VicionX
		if tx.IsVicionXApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateVicionXApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("VicionXApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
