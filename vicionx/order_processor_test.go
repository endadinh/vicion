...
func Test_getCancelFee(t *testing.T) {
	...
	vicionx.SetTokenDecimal(testTokenA, common.BasePrice)
	// tokenB has decimal 10^8
	tokenBDecimal := new(big.Int).Exp(big.NewInt(10), big.NewInt(8), nil)
	vicionx.SetTokenDecimal(testTokenB, tokenBDecimal)

	// set tokenAPrice = 1 VICION
	...
	tests := []struct {
		...
		// zero fee test: SELL
		{
			"TokenA/VICION zero fee test: SELL",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
		...
		// zero fee test: BUY
		{
			"TokenA/VICION zero fee test: BUY",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  testTokenA,
		...
		// test getCancelFee: SELL
		{
			"TokenA/VICION test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
		...
		// test getCancelFee:: BUY
		{
			"TokenA/VICION test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
		...
		// zero fee test: SELL
		{
			"VICION/TokenA zero fee test: SELL",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.TomoNativeAddress),
		...
		// zero fee test: BUY
		{
			"VICION/TokenA zero fee test: BUY",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.TomoNativeAddress),
		...
		// test getCancelFee: SELL
		{
			"VICION/TokenA test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.TomoNativeAddress),
		...
		// test getCancelFee:: BUY
		{
			"VICION/TokenA test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, _ := vicionx.getCancelFee(nil, nil, tradingStateDb, tt.args.order, tt.args.feeRate); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("getCancelFee() = %v, quantity %v", got, tt.want)
			}
		})
	}

	// testcase: can't get price of token in VICION
	testTokenC := common.HexToAddress("0x1200000000000000000000000000000000000004")
	vicionx.SetTokenDecimal(testTokenC, big.NewInt(1))
	...
	if fee, _ := vicionx.getCancelFee(nil, nil, tradingStateDb, tokenCOrder.order, tokenCOrder.feeRate); fee != nil && fee.Sign() != 0 {
	...
	vicionx.SetTokenDecimal(testTokenD, big.NewInt(0))
	...
	if fee, _ := vicionx.getCancelFee(nil, nil, tradingStateDb, tokenDOrder.order, tokenDOrder.feeRate); fee != nil && fee.Sign() != 0 {
		t.Errorf("getCancelFee() = %v, want %v", fee, common.Big0)
	}
