...
type VicionXManagedState struct {
	*TradingStateDB
	mu        sync.RWMutex
	exchanges map[common.Hash]*exchanges
}

// VicionXManagedState returns a new managed state with the statedb as it's backing layer
func ManageState(statedb *TradingStateDB) *VicionXManagedState {
	return &VicionXManagedState{
...
func (ms *VicionXManagedState) SetState(statedb *TradingStateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *VicionXManagedState) RemoveNonce(addr common.Hash, n uint64) {
...
func (ms *VicionXManagedState) NewNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *VicionXManagedState) GetNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if ms.hasAccount(addr) {
		account := ms.getAccount(addr)
...
func (ms *VicionXManagedState) SetNonce(addr common.Hash, nonce uint64) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *VicionXManagedState) HasAccount(addr common.Hash) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *VicionXManagedState) getAccount(addr common.Hash) *exchanges {
