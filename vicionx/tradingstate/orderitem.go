...
func VerifyBalance(statedb *state.StateDB, vicionxStateDb *TradingStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken().String() != common.TomoNativeAddress {
		quotePrice = vicionxStateDb.GetLastPrice(GetTradingOrderBookHash(order.QuoteToken(), common.HexToAddress(common.TomoNativeAddress)))
		log.Debug("TryGet quotePrice QuoteToken/VICION", "quotePrice", quotePrice)
		if quotePrice == nil || quotePrice.Sign() == 0 {
			inversePrice := vicionxStateDb.GetLastPrice(GetTradingOrderBookHash(common.HexToAddress(common.TomoNativeAddress), order.QuoteToken()))
			...
			if inversePrice != nil && inversePrice.Sign() > 0 {
				...
				log.Debug("TryGet quotePrice after get inversePrice VICION/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteDecimal)
			}
		}
	} else {
