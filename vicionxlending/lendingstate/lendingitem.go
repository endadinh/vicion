...
import (
	...
	"github.com/vicion/vicion/crypto/sha3"
)
...
func VerifyBalance(isVicionXLendingFork bool, statedb *state.StateDB, lendingStateDb *LendingStateDB,
	orderType, side, status string, userAddress, relayer, lendingToken, collateralToken common.Address,
	quantity, lendingTokenDecimal, collateralTokenDecimal, lendTokenVICIONPrice, collateralPrice *big.Int,
	...
	case Market, Limit:
		...
		case Investing:
			...
			case LendingStatusNew:
				...
				if lendTokenVICIONPrice != nil && lendTokenVICIONPrice.Sign() > 0 {
					...
					defaultFeeInVICION := common.Big0
					if lendingToken.String() != common.TomoNativeAddress {
						defaultFeeInVICION = new(big.Int).Mul(defaultFee, lendTokenVICIONPrice)
						defaultFeeInVICION = new(big.Int).Div(defaultFeeInVICION, lendingTokenDecimal)
					} else {
						defaultFeeInVICION = defaultFee
					}
					if defaultFeeInVICION.Cmp(common.RelayerLendingFee) <= 0 {
		...
		case Borrowing:
			...
			case LendingStatusNew:
				...
				settleBalanceResult, err := GetSettleBalance(isVicionXLendingFork, Borrowing, lendTokenVICIONPrice, collateralPrice, depositRate, borrowingFeeRate, lendingToken, collateralToken, lendingTokenDecimal, collateralTokenDecimal, quantity)
				if err != nil {
					return err
				}
