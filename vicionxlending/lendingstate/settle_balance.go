...
const DefaultFeeRate = 100 // 100 / VicionXBaseFee = 100 / 10000 = 1%
var (
...
func GetSettleBalance(isVicionXLendingFork bool,
	takerSide string,
	...
	defaultFee = new(big.Int).Div(defaultFee, common.VicionXBaseFee)

	var result *LendingSettleBalance
	//result = map[common.Address]map[string]interface{}{}

	if !isVicionXLendingFork {
		if takerSide == Borrowing {
			...
			takerFee = new(big.Int).Div(takerFee, common.VicionXBaseFee)
			...
			if lendingToken.String() != common.TomoNativeAddress && lendTokenVICIONPrice != nil && lendTokenVICIONPrice.Cmp(common.Big0) > 0 {
				exTakerReceivedFee := new(big.Int).Mul(takerFee, lendTokenVICIONPrice)
				exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, lendTokenDecimal)

				defaultFeeInVICION := new(big.Int).Mul(defaultFee, lendTokenVICIONPrice)
				defaultFeeInVICION = new(big.Int).Div(defaultFeeInVICION, lendTokenDecimal)

				if (exTakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInVICION.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("takerFee too small", "quantityToLend", quantityToLend, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInVICION", defaultFeeInVICION)
					return result, ErrQuantityTradeTooSmall
				}
			...
			if lendingToken.String() != common.TomoNativeAddress && lendTokenVICIONPrice != nil && lendTokenVICIONPrice.Cmp(common.Big0) > 0 {
				exMakerReceivedFee := new(big.Int).Mul(makerFee, lendTokenVICIONPrice)
				exMakerReceivedFee = new(big.Int).Div(exMakerReceivedFee, lendTokenDecimal)

				defaultFeeInVICION := new(big.Int).Mul(defaultFee, lendTokenVICIONPrice)
				defaultFeeInVICION = new(big.Int).Div(defaultFeeInVICION, lendTokenDecimal)

				if (exMakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInVICION.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("makerFee too small", "quantityToLend", quantityToLend, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInVICION", defaultFeeInVICION)
					return result, ErrQuantityTradeTooSmall
				}
			} else if lendingToken.String() == common.TomoNativeAddress {
		...
		borrowFee = new(big.Int).Div(borrowFee, common.VicionXBaseFee)
		...
		if lendingToken.String() != common.TomoNativeAddress && lendTokenVICIONPrice != nil && lendTokenVICIONPrice.Cmp(common.Big0) > 0 {
			...
			exReceivedFee := new(big.Int).Mul(borrowFee, lendTokenVICIONPrice)
			exReceivedFee = new(big.Int).Div(exReceivedFee, lendTokenDecimal)

			defaultFeeInVICION := new(big.Int).Mul(defaultFee, lendTokenVICIONPrice)
			defaultFeeInVICION = new(big.Int).Div(defaultFeeInVICION, lendTokenDecimal)

			if (exReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exReceivedFee.Sign() > 0) || defaultFeeInVICION.Cmp(common.RelayerLendingFee) <= 0 {
				log.Debug("takerFee too small", "quantityToLend", quantityToLend, "borrowFee", borrowFee, "exReceivedFee", exReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInVICION", defaultFeeInVICION)
				return result, ErrQuantityTradeTooSmall
			}
		} else if lendingToken.String() == common.TomoNativeAddress {
