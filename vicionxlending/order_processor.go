...
func (l *Lending) processOrderList(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, lendingStateDB *lendingstate.LendingStateDB, tradingStateDb *tradingstate.TradingStateDB, side string, lendingOrderBook common.Hash, Interest *big.Int, quantityStillToTrade *big.Int, order *lendingstate.LendingItem) (*big.Int, []*lendingstate.LendingTrade, []*lendingstate.LendingItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		lendTokenVICIONPrice, collateralPrice, err := l.GetCollateralPrices(header, chain, statedb, tradingStateDb, collateralToken, order.LendingToken)
		if err != nil {
			return nil, nil, nil, err
		}
		if lendTokenVICIONPrice == nil || lendTokenVICIONPrice.Sign() <= 0 {
		...
		tradedQuantity, collateralLockedAmount, rejectMaker, settleBalanceResult, err := l.getLendQuantity(lendTokenVICIONPrice, collateralPrice, depositRate, borrowFee, coinbase, chain, header, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (l *Lending) getLendQuantity(
	...
	LendingTokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, makerOrder.LendingToken)
	...
	collateralTokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, collateralToken)
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		isVicionXLendingFork := chain.Config().IsTIPVicionXLending(header.Number)
		settleBalanceResult, err := lendingstate.GetSettleBalance(isVicionXLendingFork, takerOrder.Side, lendTokenVICIONPrice, collateralPrice, depositRate, borrowFee, lendToken, collateralToken, LendingTokenDecimal, collateralTokenDecimal, quantity)
...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	lendTokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, originOrder.LendingToken)
	...
	if originOrder.Side == lendingstate.Borrowing {
		...
		collateralTokenDecimal, err = l.vicionx.GetTokenDecimal(chain, statedb, originOrder.CollateralToken)
	...
	if !chain.Config().IsTIPVicionXCancellationFee(header.Number) {
		...
		tokenCancelFee, tokenPriceInVICION = l.getCancelFee(chain, statedb, tradingStateDb, &originOrder, feeRate)
	}
...
func (l *Lending) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *lendingstate.LendingItem, feeRate *big.Int) (*big.Int, *big.Int) {
	...
	cancelFee, tokenPriceInVICION := common.Big0, common.Big0
	var err error
	if order.Side == lendingstate.Investing {
		cancelFee, tokenPriceInVICION, err = l.vicionx.ConvertVICIONToToken(chain, statedb, tradingStateDb, order.LendingToken, common.RelayerLendingCancelFee)
	} else {
		cancelFee, tokenPriceInVICION, err = l.vicionx.ConvertVICIONToToken(chain, statedb, tradingStateDb, order.CollateralToken, common.RelayerLendingCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
...
func (l *Lending) GetMediumTradePriceBeforeEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
	...
	if price != nil && price.Sign() > 0 {
		...
		if inversePrice != nil && inversePrice.Sign() > 0 {
			quoteTokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, quoteToken)
			if err != nil || quoteTokenDecimal.Sign() == 0 {
				return nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", quoteToken.String(), err)
			}
			baseTokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, baseToken)
...
// - Have direct pair in VicionX: lendToken/CollateralToken or CollateralToken/LendToken
// - Have pairs with VICION:
// -  lendToken/VICION and CollateralToken/VICION
// -  VICION/lendToken and VICION/CollateralToken
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	// lendTokenVICIONPrice: price of ticker lendToken/VICION
	// collateralVICIONPrice: price of ticker collateralToken/VICION
	...
	lendTokenVICIONPrice, err := l.GetVICIONBasePrices(header, chain, statedb, tradingStateDb, lendingToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralPriceUpdatedFromContract {
		...
		return lendTokenVICIONPrice, collateralPriceFromContract, nil
	}
	lendingTokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, lendingToken)
	...
	if inverseCollateralPriceUpdatedFromContract {
		...
		return lendTokenVICIONPrice, collateralPrice, nil
	}
	// if contract doesn't provide any price information
	// getting price from pair in vicionx
	...
	if lastAveragePrice != nil && lastAveragePrice.Sign() > 0 {
		log.Debug("Getting collateral/lending from direct pair in vicionx", "lendToken", lendingToken.Hex(), "collateralToken", collateralToken.Hex(), "price", lastAveragePrice)
		return lendTokenVICIONPrice, lastAveragePrice, nil
	}
	collateralVICIONPrice, err := l.GetVICIONBasePrices(header, chain, statedb, tradingStateDb, collateralToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralVICIONPrice == nil || lendTokenVICIONPrice == nil {
		return common.Big0, common.Big0, nil
	}
	// Calculate collateral/LendToken price from collateral/VICION, lendToken/VICION
	collateralPrice = new(big.Int).Mul(collateralVICIONPrice, lendingTokenDecimal)
	collateralPrice = new(big.Int).Div(collateralPrice, lendTokenVICIONPrice)
	log.Debug("GetCollateralPrices: Calculate collateral/LendToken price from collateral/VICION, lendToken/VICION", "collateralPrice", collateralPrice,
		"collateralVICIONPrice", collateralVICIONPrice, "lendingTokenDecimal", lendingTokenDecimal, "lendTokenVICIONPrice", lendTokenVICIONPrice)
	return lendTokenVICIONPrice, collateralPrice, nil
}

func (l *Lending) GetVICIONBasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {

	tokenVICIONPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, token, common.HexToAddress(common.TomoNativeAddress))
	tokenVICIONPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().Posv.Epoch == header.Number.Uint64()/chain.Config().Posv.Epoch

	if token == common.HexToAddress(common.TomoNativeAddress) {
		return common.BasePrice, nil
	} else if tokenVICIONPriceUpdatedFromContract {
		// getting lendToken price from contract first
		// otherwise, getting from vicionx lendToken/VICION
		log.Debug("Getting token/VICION price from contract", "price", tokenVICIONPriceFromContract)
		return tokenVICIONPriceFromContract, nil
	} else {
		vicionTokenPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, common.HexToAddress(common.TomoNativeAddress), token)
		vicionTokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().Posv.Epoch == header.Number.Uint64()/chain.Config().Posv.Epoch
		if vicionTokenPriceUpdatedFromContract && vicionTokenPriceFromContract != nil && vicionTokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from vicionx lendToken/VICION
			log.Debug("Getting VICION/token from contract", "price", vicionTokenPriceFromContract)
			tokenDecimal, err := l.vicionx.GetTokenDecimal(chain, statedb, token)
			...
			tokenTomoPrice = new(big.Int).Div(tokenTomoPrice, vicionTokenPriceFromContract)
			return tokenTomoPrice, nil
		}
		tokenVICIONPrice, err := l.GetMediumTradePriceBeforeEpoch(chain, statedb, tradingStateDb, token, common.HexToAddress(common.TomoNativeAddress))
		if err != nil {
			return nil, err
		}
		if tokenVICIONPrice != nil && tokenVICIONPrice.Sign() > 0 {
			log.Debug("Getting token/VICION from vicionx", "price", tokenVICIONPrice, "err", err)
			return tokenVICIONPrice, nil
		}
	}
	log.Debug("Can't getting tokenVICIONPrice ", "token", token.Hex())
	return nil, nil
}
...
func (l *Lending) ProcessRepayLendingTrade(header *types.Header, chain consensus.ChainContext, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingstateDB *tradingstate.TradingStateDB, lendingBook common.Hash, lendingTradeId uint64) (trade *lendingstate.LendingTrade, err error) {
	...
	if tokenBalance.Cmp(paymentBalance) < 0 {
		...
		var err error
		if chain.Config().IsTIPVicionXLending(header.Number) {
